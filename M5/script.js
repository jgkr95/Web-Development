function check(v, i) {
	...
	var j;
	for(var k=0;k<2;k++) {
		if(v==selectedQuestion.choices[k].option){
...
function questioner(i){
    ...
    // console.log(data.quiz[i].Q);
    document.getElementById("hint").innerHTML=data.quiz[i].hint;
    document.getElementById("option").innerHTML= '<form>'+'<label class="radio-inline">'+'<input type="radio" name="optradio" value='+'"'+ data.quiz[i].choices[0].option +'"'+'onclick="check(value,i);">'+data.quiz[i].choices[0].option+'</label>'+'<br>'+'<label class="radio-inline">'+'<input type="radio" name="optradio" value='+'"'+ data.quiz[i].choices[1].option +'"'+'onclick="check(value,i);">'+data.quiz[i].choices[1].option+'</label>'+'<br>'+'</form>'+
    '<br>'+
